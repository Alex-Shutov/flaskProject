...
def show_courier_menu(message: Message):
    ...
    try:
        ...
        bot.reply_to(
            message,
                        "🚚 Меню курьера\n\n*Создать поездку* \- Здесь отобразятся заказы, которые вы выбрали для доставки или отправки авито\.\n\n*Активыне заказы* \- заказы которые еще никто не взял в доставку, самое время это исправить\!\n\nТекущая поездка отображает, которые вы в данный момент доставляете\!",
